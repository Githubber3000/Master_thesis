...
def parse_cov_entry(cov_entry):
    if isinstance(cov_entry, str) and cov_entry.startswith("ID_"):
        n = int(cov_entry.split("_")[1])
        return np.eye(n).tolist()
    elif cov_entry.startswith("HIGH_"):
            n = int(cov_entry.split("_")[1])
            rho = 0.9
            cov = np.full((n, n), rho)
...
def extract_varying_value_from_json(png_path):
    ...
    try:
        with open(json_path, "r") as f:
            data = json.load(f)
        return float(data.get("varying_value"))
...
def generate_html_report(experiment_root_folder, report_pngs_folder, experiments, output_path):
    ...
    for group_name, configs in experiments:
        ...
        for config in configs:
            config_descr = config["config_descr"]
            png_base_path = os.path.join("results/z_html_pngs", group_name, config_descr)

            # Glob all KDE plots for this config
            kde_path = os.path.join(report_pngs_folder, group_name, config_descr, "iid_hist_kde_*.png")
            ...
            rel_kde_paths = [os.path.relpath(p, start=experiment_root_folder) for p in kde_plots]

            init_path = os.path.join(report_pngs_folder, group_name, config_descr, "init_*.png")
            init_plots = sorted(glob(init_path), key=extract_varying_value_from_json) 

            rel_init_paths = [os.path.relpath(p, start=experiment_root_folder) for p in init_plots]
            ...
            entry = {
                ...
                "init_plot_paths": rel_init_paths,
                "kde_init_pairs": list(zip(rel_kde_paths, rel_init_paths)),
            }
...
def get_initvals(init_scheme, means, num_chains, rng=None, run_id=None, init_folder=None, png_folder=None, varying_attribute=None, value=None, unimodal_init_margin = 50):
    ...
    if run_id == 1:
        ...
        chain_info_path = os.path.join(parent_folder, f"init_{varying_attribute}_{value}.json")
        chain_info_plot_path = os.path.join(parent_folder, f"init_{varying_attribute}_{value}.pdf")
        chain_info_png_path = os.path.join(png_folder, f"init_{varying_attribute}_{value}.png")
        ...
        for value in varying_values:
            ...
            if varying_attribute == "weights":
                ...
                for i, comp_params in enumerate(iid_kwargs["component_params"]):
                    ...
                    iid_kwargs["component_params"][i]["cov"] = build_correlation_cov_matrix(posterior_dim, value)
    ...
    # Single posterior case
        ...
        for value in varying_values:
            ...
            if varying_attribute == "dimension_mv":
                ...
                posterior_dim = get_posterior_dim(posterior_type, iid_kwargs)
                iid_kwargs["cov"] = build_correlation_cov_matrix(posterior_dim, value)
                save_adjusted_posterior_config(
                        iid_kwargs,
                        folder=iid_posteriors_folder,
                        dim_value=value
...
def compute_and_store_iid_stats(
    ...
    # set projections for SWD based on dimensionality, for 1D use 1, else 10*dim or max 500
    dim = get_posterior_dim(posterior_type, posterior_kwargs)
    if dim == 1:
        projections = 1
    else:
        projections = max(50, min(10 * dim, 500))
    print(f"Using {projections} projections for SWD (dim={dim})")
        ...
        for value in varying_values:
            ...
            if posterior_type == "Mixture":
                ...
                if varying_attribute == "dimension_mv":
                    ...
                    posterior_kwargs["cov"] = build_correlation_cov_matrix(posterior_dim, value)
                    if run_id == 1:
                        save_adjusted_posterior_config(
                            posterior_kwargs,
                            folder=regular_posteriors_folder,
            ...
            if init_scheme is not None:
                    ...
                    initvals = get_initvals(init_scheme, means, num_chains, rng, run_id, init_value_folder, png_folder, varying_attribute, value, unimodal_init_margin=unimodal_init_margin)
            ...
            for sampler_name in samplers:
                ...
                trace = posterior.run_sampling(
                    ...
                    means=means, 
                    posterior_type=posterior_type, 
                    run_random_seed=run_random_seed
                )
                ...
                # Only compute Wasserstein distance and MMD if we have iid_samples
                if posterior_type != "Custom":
                    ...
                    mcmc_vs_iid_swd = sliced_wasserstein_distance(posterior_samples, iid_batch, L=projections, rng=run_rng)
                    mmd_rff_value = compute_mmd_rff(posterior_samples, iid_batch, D=500, sigma=1.0, rng=run_rng)

                    print(f"SWD projections: {projections} for dim={dim}")
...
experiment_name = "New_Experiment_all_attr"
config_names = ["Multi", "Tails", "Dim", "Corr", "Skew"]
